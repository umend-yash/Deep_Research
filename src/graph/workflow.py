...
agent_path = Path.cwd().parent /  "agents"
if str(agent_path) not in sys.path:
    sys.path.insert(0, str(agent_path))

from routeragent import router
from llm_chat_bot_agent import llm_chat_bot
from search_duckduck_go_agent import search_over_intenet_with_ddg
...
graph.add_node("search_over_intenet_with_ddg",search_over_intenet_with_ddg)
...
graph.add_edge("get_relevent_query","search_over_intenet_with_ddg")
graph.add_edge("search_over_intenet_with_ddg","web_data_fetch")
